...
class field(App):
    def __init__(self, *args, **kwargs):
        if not 'editing_mode' in kwargs.keys():
            super(field, self).__init__(*args, static_file_path={'my_res':'./res/'})
    ...
    def main(self):
        return field.construct_ui(self)
...
#Configuration
configuration = {'config_project_name': 'field', 'config_address': '0.0.0.0', 'config_port': 8081, 'config_multiple_instance': True, 'config_enable_file_cache': True, 'config_start_browser': True, 'config_resourcepath': './res/'}

if __name__ == "__main__":
    # start(MyApp,address='127.0.0.1', port=8081, multiple_instance=False,enable_file_cache=True, update_interval=0.1, start_browser=True)
    start(field, address=configuration['config_address'], port=configuration['config_port'], 
